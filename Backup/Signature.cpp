...
bool CreateFunctionSig(func_t* pfn, SIG* outSig, ea_t* sig_ea, UINT32* offset)
{
...
void CreateFunctionSig()
{
	...
	ea_t* sig_ea;
	UINT32* offset;
	bool success = CreateFunctionSig(pfn, outSig, sig_ea, offset);
	if (success)
	{
		msg("Function ");
		OutputSignature(*outSig, *sig_ea, *offset);
...
void CreateFunctionSigInBulk()
{
	...
	msg("Total functions: ");
	msg(totalFunctionCount);
	return;

	for (size_t i = 0; i < totalFunctionCount - 1; i++)
	{
		auto funcData = getn_func(i);

		SIG* outSig;
		ea_t* sig_ea;
		UINT32* offset;
		bool success = CreateFunctionSig(funcData, outSig, sig_ea, offset);
		if (success)
		{
			msg("Function ");
			OutputSignature(*outSig, *sig_ea, *offset);
